...
class MessageLogger:
    """A simplified interface to Evazan AIDB so that we can log messages
    without having to share details about the user_id and the thread_id
    """

    def __init__(self, db: "Evazan AIDB", user_id: int, thread_id: int, trace_id: int = None, to_whatsapp: bool = False) -> None:
...
class Evazan AIDB:
    ...
    def store_quran_answer(
        ...
        ayah: int,
        question: str,
        evazan_ai_answer: str,
    ):
        insert_cmd = """
        INSERT INTO quran_answers (surah, ayah, question, evazan_ai_answer, review_result, final_answer)
        VALUES (%s, %s, %s, %s, 'pending', NULL)
        """
        self._execute_query(insert_cmd, (surah, ayah, question, evazan_ai_answer))
