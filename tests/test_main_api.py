...
from evazan_ai.evazan_ai_db import Evazan AIDB
from evazan_ai.evazan_ai_logger import get_logger
from evazan_ai.app.main_api import app
...
def register_user():
    ...
    response = client.post(
        ...
        headers={"x-mobile-evazan_ai": "ANSARI"},
        json={
            "email": email,
...
async def test_cors():
    # Test CORS
    allowed_origin = "https://evazan_ai.chat"
...
async def test_add_feedback(login_user, create_thread):
    ...
    message_data = {"role": "user", "content": "Assalamu Alaikum Evazan AI! How are you?"}
    response = client.post(
        f"/api/v2/threads/{create_thread}",
        headers={
            "Authorization": f"Bearer {login_user['access_token']}",
